...
import { testimonials } from "./testtimonails_people";
      ...
      <div className="slider-container relative mt-10 w-full ">
        ...
        >
          {testimonials.map((testimonial, index) => {
            ...
            let imageAvatar = `https://avatar.iran.liara.run/username?username=${fullName}`;
            return (
              <div
                key={index}
                className="card bg-white max-w-[520px] flex-shrink-0 rounded-md shadow-lg ml-9"
                style={{
                  flex: "0 0 100%",
                }}
              >
                ...
                <p className="text-gray-600 p-5">{testimonial.description}</p>
                <div className="user-section p-5 flex items-center gap-3 text-black">
                  <Image
                    // src={"/reviews/user1.png"}
                    src={imageAvatar}
                    width={70}
                    height={70}
                    alt="User profile"
